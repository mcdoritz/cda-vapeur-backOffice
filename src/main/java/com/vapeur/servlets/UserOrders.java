	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			if(checkAdmin(session)) {
				prln("servlet userOrders : admin loggué");
				...
				if(request.getParameter("id") != null && Integer.valueOf(request.getParameter("id")) > 0) {
					...
					request.setAttribute("ordersList", orderdao.readAllByUserId(user_id));
				...
				request.getRequestDispatcher("WEB-INF/app/userOrders.jsp").forward(request, response);
			}else {
